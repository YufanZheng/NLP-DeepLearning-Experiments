...
# Random shuffle
pos_index = range(len(mr_positive_list))
neg_index = range(len(mr_negative_list))
random.shuffle(pos_index)
random.shuffle(neg_index)
mr_positive_list = map(lambda x: mr_positive_list[x], pos_index)
mr_negative_list = map(lambda x: mr_negative_list[x], neg_index)

with file('mr-polarity.pos', 'w') as fout:
    for line in mr_positive_list:
        fout.writelines(line+'\n')
with file('mr-polarity.neg', 'w') as fout:
