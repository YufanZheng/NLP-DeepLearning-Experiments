...
class WordEmbedding(object):
    ...
    def __init__(self, fname):
        '''
        @fname: String. Folder path to the wiki-word embedding files
       '''
        ...
        with zipfile.ZipFile(fname) as z:
            ...
            with open(filename) as fin:
                line = fin.readline()
                (self._dict_size, self._embed_dim) = [int(s) for s in
                        line.split()]
                self._embedding = np.zeros((self._dict_size,
                        self._embed_dim), dtype=np.float32)
                ...
                for i in range(self._dict_size):
                    ...
                    self._embedding[i, :] = np.array([float(x) for x in
                            line[1:]])
